        ...
        def _orig_lookup(address, exact=False, indirect=False):
            ...
            if not exact and not indirect:
                if reference is not None and address == reference[0]:
                    return reference[2]
            dest = _ilt_destination(self, address)
            lookup_addr = dest if dest is not None else address
            dest_name = orig_lookup(lookup_addr, exact=exact, indirect=indirect)
            if dest_name is not None:
                if dest_name.endswith(" (IMPORT_THUNK)"):
                    return dest_name[:-15] + " (IMPORT)"
                return dest_name
            dest_entity = self.db.get(ImageId.ORIG, lookup_addr)
            if dest_entity is not None:
                resolved = dest_entity.match_name() or dest_entity.best_name()
                if resolved is not None:
                    return resolved
            if match.name and "::`scalar deleting destructor'" in match.name:
                class_name = match.name.split("::`scalar deleting destructor'")[0]
                return f"{class_name}::~{class_name} (FUNCTION)"
            ...
            if not indirect:
                ...
                if destination is not None:
                    ...
                    if entity is not None:
                        resolved_name = entity.match_name() or entity.best_name()
                        if resolved_name is not None:
                            return resolved_name
                if name is not None and name.startswith("Thunk of '") and name.endswith("'"):
                    return name[10:-1]
            return name
