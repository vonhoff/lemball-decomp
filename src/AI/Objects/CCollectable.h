#ifndef LEMBALL_AI_OBJECTS_CCOLLECTABLE_H
#define LEMBALL_AI_OBJECTS_CCOLLECTABLE_H

#include "../Base/CGlobalGameObject.h" // complete type
#include "AI/Base/ObjectTypes.h"

// SIZE 0x13c
// VTABLE: LEMBALL 0x004959c0
class CCollectable : public CGlobalGameObject {
public:
	CCollectable(int p_x, int p_y, int p_z, eObjectType p_objectType);
	virtual bool Process();    // vtable+0x14
	virtual void DoActivate(); // vtable+0x10c
	virtual void Restart();    // vtable+0x104
	virtual void SetSfx();     // vtable+0x110
	virtual int Collected();   // vtable+0x114
	virtual ~CCollectable();   // vtable+0x00
	CCollectable();

	friend class CCollectableManager;

private:
	unsigned int m_enabled; // 0x138
};

// SYNTHETIC: LEMBALL 0x00423060
// CCollectable::`scalar deleting destructor'

#endif
