...
TargetDisplayDibDriver::TargetDisplayDibDriver(const CVsSize& p_size)
{
	m_ready = 0;
...
TargetDisplayDibDriver::~TargetDisplayDibDriver()
{
...
unsigned int TargetDisplayDibDriver::UpdateDIBColourTable(TargetDrawingContext* p_context,
														  unsigned int p_start,
														  unsigned int p_count,
														  void* p_colours)
{
	SetDIBColorTable((HDC) ((TargetGdiDrawingContext*) p_context)->m_hDC, p_start, p_count, (RGBQUAD*) p_colours);
	_outp(0x3c8, p_start);
...
int TargetDisplayDibDriver::BitBltContexts(TargetDrawingContext* p_destination,
										   CVsRect* p_rect,
										   ...
										   CVsPoint* p_position)
{
	...
	int stride = ((TargetDibContext*) ((TargetGdiDrawingContext*) p_source)->m_hBitmap)->GetStride();
	unsigned char* destination = m_frameBuffer + clipped.m_y * m_screenSize.m_width + clipped.m_x;
	unsigned char* source = ((TargetDibContext*) ((TargetGdiDrawingContext*) p_source)->m_hBitmap)->GetBits();
...
int TargetDisplayDibDriver::StretchBltContexts(TargetDrawingContext* p_destination,
											   CVsRect* p_rect,
											   TargetDrawingContext* p_source,
