...
TargetDirectDrawDriver::TargetDirectDrawDriver(VsSize* p_size, int p_fullScreen)
{
	...
	*directDraw = 0;
	...
	if (result != 0) {
		TargetDirectDrawCreateFailureHook(0, result);
		return;
	}
	if (p_fullScreen != 0) {
...
TargetDirectDrawDriver::~TargetDirectDrawDriver()
{
	if (m_directDraw != 0) {
										   ...
										   VsPoint* p_sourcePosition)
{
	...
	long result = m_primarySurface->BltFast(
		clipped.m_x,
		clipped.m_y,
		((TargetDirectDrawSurfaceContext*) m_contextSurfaces[((TargetDirectDrawContext*) p_source)->m_surfaceIndex])
			->m_surface,
		&source,
		0);
	if (result != 0) {
...
bool TargetDirectDrawDriver::HasPalette()
{
