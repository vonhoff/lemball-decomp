...
MogDir::MogDir(unsigned long p_fileOffset)
{
	...
	m_firstIndex = g_chunkIndex;
	m_iteratorIndex = g_chunkIndex;
...
void MogDir::GetChunkInfo(ChunkInfo* p_info)
{
	...
	VsSeek(g_pMogFile, (m_iteratorIndex * 4 + 4) * kDirectoryEntryStride + m_directoryEndOffset, kSeekSet);
	p_info->m_next = 0;
	p_info->m_child = 0;
	p_info->m_directory = 0;
	dataOffset = InputDword();
	p_info->m_data = m_directoryData + (dataOffset - m_payloadStartOffset);
...
MogDir* MogDir::GetNextDir()
{
	...
	if (m_rootChunk == 0) {
		...
		if (chunk.m_info->m_type == kChunkDirc) {
			chunk.m_info->m_directory = new MogDir(chunk.m_info->m_fileOffset);
	...
	if (m_currentDirChunk->m_child != 0) {
		m_currentDirIndex = m_currentDirChunk->m_childIndex;
		m_currentDirChunk = m_currentDirChunk->m_child;
		return m_currentDirChunk->m_directory;
	}
	m_iteratorIndex = m_currentDirIndex;
	...
	if (chunk.m_info->m_type == kChunkDirc) {
		...
		m_currentDirIndex = chunk.m_index;
...
void MogDir::FindNext(Chunk& p_chunk, unsigned int p_type)
{
	...
	do {
		if (m_iteratorIndex == -1) {
			...
			if (m_chunkCount - m_iteratorIndex == 1) {
				exhausted = 1;
				break;
			}
		...
		m_iteratorIndex = *nextIndex;
		m_iteratorChunk = *(ChunkInfo**) (nextIndex + 1);
		if (p_type == kAnyChunkType) {
			break;
		}
	} while (m_iteratorChunk->m_type != p_type);

	if (p_type == kAnyChunkType || m_iteratorChunk->m_type == p_type) {
		if (exhausted == 0) {
			p_chunk.m_index = m_iteratorIndex;
			p_chunk.m_info = m_iteratorChunk;
			return;
		}
	}
	p_chunk.m_info = 0;
