...
Each is genuinely missing but infrastructure-bound — needs door ctor `0x401eba`→`0x40d470` ✅ (exists), door vtable Restart `[+0x104]`, and/or global tile-map `0x4a74b4` (`+0xc` tile data, `+0x10` w, `+0x14` h). Not isolated wins — reconstruct as a subsystem. DONE: CDoorManager::Restart (100%), Initialise (45.5%), LinkScfTramChunkManagerView::Restart (100% eff). PaintGun/Ice Manager Restart pre-existing (ActivatePgunChunkObjects/CIceManager::Restart). Next: CDoorManager::Add/Open/LoadLevel, then CDoor::Set/Delete/collision (tile-map 0x4a74b4 heavy).
