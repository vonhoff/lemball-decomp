...
All twenty-six portable Macintosh `CPlayerLemmingGroupManager` methods align with Windows `0x00418400..0x00419490` through PLAS manager/stream/group vtables, exact this-adjustment, five-child allocation, dead-history and network-player fields, signatures, inverse operations, loader formats, callers, and render stride. `GetDead`, `GetLeaderPos`, `ProcessDead`, `IsLemmingPlayerControlled`, `GetPlayerControlledGroup`, `RemoveWaypointsFromCurrentGroup`, `InitialiseNetwork`, and `HasSFXChanged` now have genuine ownership in `src/AI/CPlayerLemmingGroupManager.cpp`; the neutral active-state and waypoint TUs plus obsolete annotation wrappers were deleted, while represented ILT boundaries correlate directly to the genuine members. `ProcessDead` is an exact no-op; its target-only ILT remains intentionally unmapped until `Process` is reconstructed rather than recreating it with a source wrapper. The checkpoint remains `184/36`; aggregate objdiff remains `1805/3415`, while matched code rises from `61930` to `61975` bytes. Five target-only interleavings and deleting wrapper `0x00419930` remain explicitly excluded.
