...
All six portable `CNode` and six portable `CNodeManager` methods align with Windows `0x00421180..0x00421470` through exact ABI, 0x14-byte node stride, counted allocation, fixed-point coordinates, neighbour-pair storage, ILTs, callers, and NODE chunk parsing. Genuine `CNode` constructor, `Restart`, destructor, `Initialise`, and `AddANeighbour` ownership now lives in `src/AI/LVNODE.CPP` as `100%` matches. Windows-only tile-coordinate overloads and record helpers remain excluded.
