...
The five portable methods map at `0x0041D600..0x0041D7B0`. Shared narrow `CBalloon` layout lives in `CBalloon.h`; genuine `Restart` and `Activate` live in `LVSAVEPOS.CPP`, while genuine `ActivatePosition` lives in `LVPOSVT.CPP`; all three are `100%` matches. Exact 68K and Windows bodies agree on randomized timestamp restart, terrain refresh, network action `0x2A`, state `0x1A`, target permission/callback dispatch, score `50`, and unchanged activation-position output. The target permission call preserves the evidenced one-word member-pointer vtable slot and the output method preserves the Windows hidden-result-pointer ABI.
