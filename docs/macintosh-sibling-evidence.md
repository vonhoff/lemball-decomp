...
All six portable `CNode` and six portable `CNodeManager` methods align with Windows `0x00421180..0x00421470` through exact ABI, 0x14-byte node stride, counted allocation, fixed-point coordinates, neighbour-pair storage, ILTs, callers, and NODE chunk parsing. All six genuine `CNode` members plus five genuine `CNodeManager` members now live in `src/AI/LVNODE.CPP`; the manager constructor is exact, while `Restart`, `Initialise`, destructor, and `GetNodePosition` are `46.51%`, `75.47%`, `21.74%`, and `91.67%` due VC4 translation-unit register allocation and semantic relocation naming. Windows-only tile-coordinate overloads and record helpers remain excluded.
