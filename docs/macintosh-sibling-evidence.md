...
All eight portable `CHand` and nine portable `CHandManager` methods align with Windows `0x00427AD0..0x004284C0` through level chunk type `0x1F`, 0x144-byte child stride, capture states `0x17..0x1A`, contact geometry, frame timers, EFF-backed manager fields, view emission and versioned loader records. Genuine `CHand.cpp` now owns construction, `Initialise`, `Restart`, and destruction; genuine `CHandManager.cpp` owns construction, capacity-wide `Restart`, and destruction with a typed 0x40-byte layout and vtable slot. The remaining portable behavior stays tracked in its current mixed owners until reconstructed. Windows remove helper `0x00427FD0` and deleting wrappers `0x004287D0`/`0x00428800` remain separate.
