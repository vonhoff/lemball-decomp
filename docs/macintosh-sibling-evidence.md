...
All five portable `CFormationManager` methods align with Windows `0x0041A140..0x0041A320` through the 0x104-byte three-block vector table, fixed16.12 initialization, signed sine/cosine rotation of eight vectors, transformed output at `+0xC0`, and wrapped accessor. Genuine constructor, `Restart`, destructor, and `GetAVector` ownership now lives in `src/AI/CFormationManager.cpp`; the constructor is `46.15%` with external helper relocation/compiler-state differences and the other three are `100%` matches. The original constructor ILT now bridges to the genuine constructor. Windows iterator helpers `0x0041A2E0` and `0x0041A300` remain separate.
