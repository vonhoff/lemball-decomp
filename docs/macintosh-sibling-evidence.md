...
All six portable `CNode` and six portable `CNodeManager` methods align with Windows `0x00421180..0x00421470` through exact ABI, 0x14-byte node stride, counted allocation, fixed-point coordinates, neighbour-pair storage, ILTs, callers, and NODE chunk parsing. All six genuine `CNode` members plus the genuine `CNodeManager` constructor and `Restart` now live in `src/AI/LVNODE.CPP`; the constructor and node members are exact, while manager `Restart` is `46.51%` due VC4 translation-unit register allocation. Windows-only tile-coordinate overloads and record helpers remain excluded.
