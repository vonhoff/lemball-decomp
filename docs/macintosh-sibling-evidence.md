...
The 12 portable `CMaze` methods and global `Direction` map at `0x00423090..0x004238B0`. Windows evidence preserves the `0x103C` layout, row-allocated 16-bit passability/distance grid, two fixed-stride frontier banks, edge-mask-constrained nine-neighbor relaxation, reference arguments, bounded breadth expansion, direction coding, and turn-only solution output. `CMaze::CMaze(CMap*)`, `CMaze::~CMaze()`, `CMaze::ReInitialise()`, `CMaze::Initialise()`, `CMaze::CalcNewDistance(int, int)`, and `CMaze::FindSquare(unsigned short, int&, int&)` now have genuine ownership in `src/AI/CMaze.cpp`; they match `100%` exact, `55.56%`, `40.00%`, `100%` effective, `88.89%`, and `90.20%`. The remaining methods stay in `LEVELRUN.CPP` and `LVPATHDIR.CPP` pending reconstruction. Macintosh-only `AdvanceBit`, `ChangeAddr`, and `ChangeNextAddr` remain unresolved pending standalone Windows identities.
