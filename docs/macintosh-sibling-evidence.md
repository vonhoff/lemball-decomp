...
All thirty-nine portable Macintosh `CPlayerLemming` methods align with Windows `0x0040ECB0..0x004102D0` through PLAS lifecycle/layout, signatures, ILTs/callers, vtable slots, stream inverse pairs, group/object fields, distinctive balloon codes, fixed-point coordinates, RNG constants, and state transitions. Both `SetGroup` overloads, `GetGroup`, and `SetGroupLeader` now live in genuine `AI/CPlayerLemming.h/.cpp` ownership with typed fields and an exact `0x22c` layout; their obsolete neutral wrappers were deleted and real callers use the class. The Macintosh `unsigned char` flags retain the evidenced widened Windows `int` ABI. The same ownership batch deleted the mixed `VSMath.cpp/.h`: `CVSMath::SqRoot` and the canonical `CVSPoint` now have one-class translation units, while evidenced target-only fixed-vector and signed-trig helpers are isolated separately. Against `b32bb16`, matched code rose from 61,855 to 62,484 bytes and exact functions from 1,805 to 1,807; the fixed baseline changed from 184 added / 36 lost to 185 / 35. Exact gains were `0x0042E500`, `0x00430110`, `0x00430A50`, `0x00430B30`, `0x0046B310`, `0x0046BA50`, and `0x0046BF70`; exact losses were `0x00406BA0`, `0x00408DC0`, `0x004305F0`, `0x00457360`, and `0x00466BD0`, a measured compiler/link-state trade from the architectural TU split. Target-only `0x0040F7E0`, deleting wrapper `0x00410B20`, and nine Macintosh-specific duplicate members remain explicitly excluded.

### CGenericGroup portable family

All twenty-six portable Macintosh `CGenericGroup` methods align with Windows `0x0041DDA0..0x0041E640` through managed-group lifecycle/layout, vtable slots, iterator and membership inverse pairs, bounds sentinels, formation geometry, waypoint dispatch, intersection behavior, and render-record stride. Genuine `GetElementsInGroup`, `GetFirstElementInGroup`, `GetNextElementInGroup`, `GetCurrentElementInGroup`, and `GetNthElementInGroup` replace the neutral child-iterator owner and each match `100%`; the three existing iterator ILT callers remain exact. Genuine `SwapElements`, `AddElementToGroup`, `RemoveElementFromGroup`, and the pointer/id `ConfirmElementIsInGroup` overloads are respectively `51.61%`, `100%`, `100%`, `57.89%`, and `64.00%`, preserving the typed ten-slot table, dirty flag `+0x164`, and evidenced 16-bit child field `+0x6A`; the player-group add caller remains exact after using the genuine add member. The former neutral add ILT at `0x00401AC3` is retained in metadata for the target boundary but the genuine scoped relocation is not emitted into the same VC4 linker slot. Genuine aggregate/reference `GetBoundingBox` overloads and `CalculateBoundingBox` now own the four-short bounds at `+0x158`; they preserve the hidden aggregate output, exact `CVSRect&` signature, fixed-point `x/y` reads at child `+0x9C/+0xA0`, sentinel extrema, and short truncation, matching `16.67%`, `54.55%`, and `25.42%` respectively. Genuine `OverideExistingWaypoints(AICOORD)`, `SetFormationIndex(int)`, and `GetFormationIndex()` replace the neutral no-op and raw field helpers at `0x0041E3C0/0x0041E400/0x0041E410`; all three are exact, preserving the Macintosh spelling, by-value coordinate ABI, and formation index at `+0x160`. Genuine `ClearExistingWaypoints()` at `0x0041E3D0` is effective-exact: it clears the command queue count at group `+0x70`, iterates through vslots `0x114/0x118`, and calls the already genuine `CGameObject::ResetInstructions()` boundary. Its target ILT `0x00402446` is mapped to that genuine member, although VC4 does not retain the same reconstructed linker slot after the neutral helper name is removed. Genuine `AddNewWaypoint(AICOORD,CFormationManager*)` at `0x0041E290` stores the shared formation context and appends command type `1` to the existing typed 20-byte `CGameObjectCommandQueue` when capacity permits; its by-value coordinate and pointer ABI are preserved at `25.00%`. Genuine `SendNewWaypoint(AICOORD)` at `0x0041E2E0` reuses the recovered facing helper, genuine `CFormationManager::TransformFormation`/`GetAVector`, typed first/count/next group vslots, and genuine `CGameObject::AddDestination`; it matches `55.12%`, with remaining differences limited to VC4 register allocation and scoped relocations. Genuine `CheckGroupIntersection(CVSRect*,AICOORD*)` and `GetViewData(CViewData*)` now own `0x0041E530/0x0041E640`: the former bounds-rejects before scanning typed child positions, while the latter emits one child virtual view record per `0x4C` bytes through first/next group iteration. They match `31.08%` and `50.85%`. The recovered family now lives in `AI/CGenericGroup.h/.cpp` rather than all-caps `LEVELVT.CPP`; `Restart` and `Process` were likewise moved out of `LVSAVEPOS.CPP` and `CEnemyGrp.cpp`, duplicate compatibility structs were deleted, and the enemy-group vtable now stores `&CGenericGroup::Process` directly instead of a free wrapper; extracted `Restart` and `Process` match `91.67%` and `63.83%`. Target-only `0x0041DF90` and deleting wrappers remain excluded pending their evidenced owners.

### CPlayerLemmingGroup portable ownership

Seven evidenced methods now live in `AI/CPlayerLemmingGroup.h/.cpp`: `Restart`, `Delete`, `AddLemmingToGroup`, `SetPlayerControlled`, `CheckPlayerControlled`, `GetFirstDeadLemming`, and `HasSFXChanged`. `Restart`, `Delete`, `AddLemmingToGroup`, and `CheckPlayerControlled` match `100%`; `SetPlayerControlled` is an effective `100%`, while typed virtual child iteration leaves the latter two at `37.84%` and `77.55%`. The neutral owners in all-caps `LEVELVT.CPP`, `LEVELVTCHILD.CPP`, and `LVGRPTGT.CPP` were deleted, including the obsolete all-caps child TU; existing ILT callers now route to genuine members. Constructor/destructor, view-data, process, use-object, removal, and waypoint methods remain unresolved and were not fabricated. After the player-control manager batch, the checkpoint is `185/34`, aggregate objdiff is `1808/3415` with `62422` matched code bytes, vtables remain exact, and datacmp reports zero issues.

### CPlayerLemmingGroupManager lifecycle core

All twenty-six portable Macintosh `CPlayerLemmingGroupManager` methods align with Windows `0x00418400..0x00419490` through PLAS manager/stream/group vtables, exact this-adjustment, five-child allocation, dead-history and network-player fields, signatures, inverse operations, loader formats, callers, and render stride. `GetDead`, `GetLeaderPos`, `ProcessDead`, `AddPlayerLemmingToGroup`, `IsLemmingPlayerControlled`, `MakeNextGroupPlayerControlled`, `MakePreviousGroupPlayerControlled`, `MakeParticularGroupPlayerControlled`, `MakeNoGroupsPlayerControlled`, `GetPlayerControlledGroup`, `AddNewWaypointToCurrentGroup`, `RemoveWaypointsFromCurrentGroup`, `InitialiseNetwork`, and `HasSFXChanged` now have genuine ownership in `src/AI/CPlayerLemmingGroupManager.cpp`; waypoint dispatch uses typed `CGenericGroup` slots `0x144`/`0x150`, the shared `CFormationManager*`, and the exact genuine `AICOORD` copy constructor instead of a neutral three-dword wrapper. The neutral active-state and waypoint TUs plus obsolete annotation wrappers were deleted, while represented ILT boundaries correlate directly to the genuine members. `ProcessDead` is an exact no-op; its target-only ILT remains intentionally unmapped until `Process` is reconstructed rather than recreating it with a source wrapper. `AddPlayerLemmingToGroup` is exact after correcting both stale ILT identities. Its added class declaration perturbs the MSVC 4.00 compiler state in `CGame.cpp`: `0x00418820` becomes exact while `GameLevelProgressState::SetUnlockedPackCap` at `0x00408FE0` changes to `60.87%`, so the checkpoint remains `184/36`; aggregate objdiff remains `1805/3415`, with `61855` matched code bytes. Five target-only interleavings and deleting wrapper `0x00419930` remain explicitly excluded. The `AddNewWaypointToCurrentGroup`/`AICOORD` ownership batch makes `0x00418A30` exact while preserving exact `0x00410B50`. Against committed `fbda158`, compiler-state/link-order effects gain exact `0x00406BA0`, `0x00408DC0`, `0x00408FE0`, `0x00418A30`, `0x0041E0C0`, and `0x0042B4A0`, while losing exact `0x0040C8F0`, `0x0041AFD0`, `0x0041B7D0`, `0x0041E050`, `0x00424C60`, `0x0042DD00`, `0x0042E500`, and `0x004662B0`; aggregate objdiff changes from `1807/3415` and `62484` matched code bytes to `1805/3415` and `62126`, while the fixed checkpoint changes from `185/35` to `181/33`. The small net fidelity regression is accepted because it replaces an absent manager member and neutral three-dword wrapper with exact evidenced Macintosh ownership and ABI. The player-control batch adds genuine `CPlayerLemmingGroup::SetPlayerControlled` at an effective `100%` and makes `MakeNextGroupPlayerControlled`, `MakePreviousGroupPlayerControlled`, `MakeParticularGroupPlayerControlled`, and `MakeNoGroupsPlayerControlled` exact, while distinguishing the player-selection index at `0x120` from the generic iterator index at `0xA8`. Against committed `f6ae7ce`, exact addresses gained are `0x00418860`, `0x004188E0`, `0x00418960`, `0x004189C0`, `0x00419440`, and `0x004662B0`; exact addresses lost to compiler-state effects are `0x00406BA0`, `0x00408FE0`, and `0x00413F80`. Aggregate objdiff improves from `1805/3415` and `62126` matched code bytes to `1808/3415` and `62422`; the fixed checkpoint changes from `181/33` to `185/34`.

### CGenericGroupManager lifecycle and iterator core

All twenty-five portable Macintosh `CGenericGroupManager` methods align with Windows `0x0041E8F0..0x0041EED0` through the 40-slot manager layout, vtable/ILT calls, exact count/index fields, nested iterator and mutation inverse pairs, signatures, allocation sizes, bounding-box/view-data strides, and intersection dispatch. Genuine group iterators `GetNumberOfGroups`, `GetFirstGroup`, `GetNextGroup`, `GetNthGroup`, and `GetCurrentGroup` are each `100%`; genuine element iterators `GetNumberOfElements`, `GetFirstElement`, `GetNextElement`, `GetCurrentElement`, and `GetNthElement` are respectively `73.91%`, `100%`, `63.33%`, `100%`, and `65.22%`. `GetGroupElementIsMemberOf` is genuine at `64.71%`; `AddNewGroup` and no-argument `CreateNewGroup` are exact, while `DeleteGroup` is genuine at `86.08%` with evidenced deleting-slot dispatch and fixed-table compaction. The genuine mutation trio `AddElementToGroup`, `RemoveElementFromGroup`, and `FindElementInGroupAndRemoveIt` is respectively `100%`, `45.28%`, and `100%`, preserving group slots `0x108/0x124/0x128`, deleting dispatch, the active-empty policy, and 40-slot compaction. Genuine `GetAllBoundingBoxes`, `GetViewData`, and `CheckGroupIntersection` are respectively `76.09%`, `67.80%`, and `73.53%`, preserving group slots `0x48/0x158/0x15C`, four-short rectangle conversion, the `0x4C` view stride, and first-hit intersection semantics. Typed one-word member-function slots replace the neutral iterator, membership, append, creation, mutation, and query helpers; the neutral membership definition in `LVACTVT.CPP` was deleted and its active-state caller now calls the shared genuine manager member at `90%`. Target-only child-destruction helper `0x0041E9A0` remains explicitly excluded.
...
### CMove3d portable family

Both portable Macintosh `CMove3d` methods now have genuine ownership in `src/AI/CMove3d.cpp`, correlated to the fixed-point segment implementation at Windows `0x0042A7D0..0x0042A8F0` through exact signatures, the `0x1C` layout, CMover ILTs, callers, and interpolation behavior. `Set` matches `73.40%`; `Position` matches `37.74%`.

### CMover portable family

All sixteen portable Macintosh `CMover` methods align with Windows `0x0042E590..0x0042F140` through exact signatures, mover vtable/ILTs, type `0x34`, the 16x16 footprint, route/timing fields, fixed-point interpolation, rider attachment inverse operations, and manager callers. The constructor is a `100%` effective match; `CMover::Restart`, `Initialise`, destructor, `IsAt`, `Switch`, and `StopObjectsMoving` have genuine exact ownership in `src/AI/CMover.cpp`; `CMover::Set`, `SetPos`, `SetUpNextNode`, `MoveObjects`, `IsOn`, and `VerifyObjects` match `59.15%`, `41.79%`, `33.51%`, `23.08%`, `48.28%`, and `28.85%` respectively; the evidenced Windows object size remains `0x1A0`. Target-only rider synchronization `0x0042EAC0` and zero callbacks `0x0042EEC0`/`0x0042EED0` remain excluded.
...
All six portable `CNode` and all six portable `CNodeManager` methods align with Windows `0x00421180..0x00421470` through exact ABI, 0x14-byte node stride, counted allocation, fixed-point coordinates, neighbour-pair storage, ILTs, callers, and NODE chunk parsing. Both complete genuine families now live in `src/AI/LVNODE.CPP`; the manager constructor is exact, while `Restart`, `Initialise`, destructor, `GetNodePosition`, and `LoadLevel` are `46.51%`, `64.15%`, `21.74%`, `91.67%`, and `94.74%` due VC4 translation-unit register allocation and semantic relocation naming. Windows-only tile-coordinate overloads and record helpers remain excluded.
...
All five portable `CFormationManager` methods align with Windows `0x0041A140..0x0041A320` through the 0x104-byte three-block vector table, fixed16.12 initialization, signed sine/cosine rotation of eight vectors, transformed output at `+0xC0`, and wrapped accessor. The complete genuine family now lives in `src/AI/CFormationManager.cpp`: constructor `46.15%`, `TransformFormation` `23.14%`, and the remaining three methods `100%`. The Windows-only fixed-point rotation helper is reconstructed at `22.07%`; current differences are helper relocation and compiler expression scheduling. The original constructor ILT bridges to the genuine constructor. Windows iterator helpers `0x0041A2E0` and `0x0041A300` remain separate.
### CHand portable families

All eight portable `CHand` and nine portable `CHandManager` methods align with Windows `0x00427AD0..0x004284C0` through level chunk type `0x1F`, 0x144-byte child stride, capture states `0x17..0x1A`, contact geometry, frame timers, EFF-backed manager fields, view emission and versioned loader records. The complete genuine families now live in `src/AI/CHand.cpp` and `src/AI/CHandManager.cpp`. `CHand` derives from the evidenced 0x124-byte `CGameObject` layout and uses named members for initial/current coordinates, target, frame time, update ticks, network state, activation/configuration flags and captured object through `+0x140`; opaque storage remains only for unresolved intervals. Its constructor, `Initialise`, `Restart`, destructor and `DoActivate` are exact; `Set`, `Process`, and `StepOn` are behaviorally reconstructed at `55.81%`, `56.80%`, and `43.14%`. The manager retains its typed 0x40-byte layout, 0x144 child traversal and typed `Restart`/`Process`/`GetViewData` vtable slots. Its constructor, `Restart`, `Add`, and destructor are exact; `Initialise`, `StepOn`, `Process`, `GetViewData`, and `LoadLevel` are reconstructed at `72.41%`, `68.29%`, `45.28%`, `83.87%`, and `36.59%`. Windows remove helper `0x00427FD0` and deleting wrappers `0x004287D0`/`0x00428800` remain separate.
...
All nine portable `CIce` and nine portable `CIceManager` methods align with Windows `0x0042CA70..0x0042DEA0` through chunk type `0x33`, a `0x188`-byte child stride, bounded endpoint geometry, movement of up to ten attached entities, terrain-contact redispatch, EFF stream payload `0x0E`, exact zero-body view behavior and versioned loader records. Genuine `CIce::Switch` now lives in `AI/CIce.cpp`; genuine `CIceManager` constructor, `Restart`, destructor, `Process`, `GetViewData`, and `Switch` live in `AI/CIceManager.cpp`, with typed `0x40` layout and vtable slots. The constructor, `Restart`, destructor, `GetViewData`, and both switch methods remain exact; `Process` remains an effective `100%` (`60%` structural). The obsolete mixed `CIceAct.cpp` and superseded manager wrappers were deleted. Count/remove helpers `0x0042D900`/`0x0042D910`, auxiliary-copy helper `0x0042DE40`, and deleting wrapper `0x0042E4D0` remain separate. Against `edee4d8`, the extraction changes the report from `1808/3415` and `62,422` exact bytes to `1809/3415` and `62,759`: exact gains at `0x00408FE0`, `0x00413F80`, `0x0041AFD0`, and `0x0041B7D0`; compiler-state losses at `0x00408830`, `0x00419440`, and `0x0042C040`; fixed checkpoint `187 added / 35 lost`.
...
All 18 portable methods map at `0x00409CA0..0x0040A490`. Windows evidence preserves the `0x2B8` object stride, `0x40` manager layout, two-coordinate bounds setup, `0x18` tracked-object capacity, contact/activation state machine, and versioned level loader. The constructor, destructor, `Restart`, `Initialise`, `Set`, `VerifyObjects`, `AddObject`, `StepOn`, `DoActivate`, `Process`, and `Load` are genuine `CInvisibleSwitch` members in `CInvisibleSwitch.cpp`; all seven manager members are genuine members in `CInvisibleSwitchManager.cpp`. Their Macintosh signatures replace the neutral compatibility views without attribution annotations. Typed member-function slots preserve both original 32-bit vtable layouts. The current ownership checkpoint is `184/34`; manager constructor and `Process` plus the reconstructed object lifecycle and state-reset members are exact, while object `Set`/`Load`, manager `Restart`, `Initialise`, and `LoadLevel` are behaviorally reconstructed at `78.02%`, `90.24%`, `54.55%`, `81.97%`, and `41.38%` respectively with relocation-name or compiler register/scheduling differences. The target-only deleting wrapper reports a semantic relocation-name mismatch, the original constructor ILT at `0x0040349F` relocates with the owning translation unit while the constructor body stays exact, and the reviewed CLift activation ownership introduces additional relocation-name/compiler-state differences in adjacent lift and unrelated bodies. Aggregate objdiff is `1807/3415`; vtables remain exact and datacmp has zero issues. The point-only object overload `0x00409D20`, manager Add overloads `0x0040A3E0`/`0x0040A440`, and deleting wrapper `0x0040AC60` are target-only and consume no portable symbols.

### CLift and CLiftManager

All 21 portable methods map at `0x00424D00..0x00425FC0`. Windows evidence preserves the `0x190` object stride, endpoint/legacy setup overloads, boundary cliff updates, eight passenger slots, timed motion states, view records, slot switching, and versioned loader while retaining split Windows ownership. `CLift::CLift()`, `CLift::~CLift()`, `CLift::CalculateCliff()`, `CLift::Edit(...)`, both `CLift::Set(...)` overloads, `CLift::Process()`, `CLift::CheckObjects()`, `CLift::StepOn(...)`, `CLift::Activate()`, and `CLift::ActivateDeactivate()` now have final ownership across `src/AI/CLift.cpp`, `src/AI/CLiftRun.cpp`, and `src/AI/CLiftAct.cpp`; all 11 portable CLift methods therefore have final class ownership. All ten portable `CLiftManager` methods now have final ownership in `src/AI/CLiftManager.cpp`: constructor, `Restart`, `Initialise`, destructor, `Process`, `StepOn`, `GetViewData`, `Switch`, `Id`, and `LoadLevel`; typed CAI callers replace the old neutral wrappers. The constructor, destructor, `Id`, and retained manager deleting wrapper are 100% exact, while `Restart`, `Initialise`, `Process`, `StepOn`, `GetViewData`, `Switch`, and `LoadLevel` are behaviorally reconstructed at `73.91%`, `83.05%`, `32.65%`, `52.00%`, `44.90%`, `96.00%`, and `9.47%` with typed manager-vtable slots, native `0x190`-stride CLift array construction, and versioned legacy/endpoint record decoding. The lifecycle pair is 100% exact, `Activate` is a 100% effective match, and `CalculateCliff`, `Edit`, legacy `Set`, endpoint `Set`, `Process`, `CheckObjects`, `StepOn`, and `ActivateDeactivate` are behaviorally reconstructed at `97.18%`, `59.11%`, `91.67%`, `24.69%`, `89.62%`, `50.67%`, `40.49%`, and `85.71%`. The setup path preserves the six-byte coordinates, unique object ID, fixed-point position, activation modes, passenger reset, tile flags/heights, signed tile division, and the evidenced end-X reuse in the second boundary loop. The required target-only deleting paths delegate through the reconstructed destructors; the manager wrapper is exact and the object wrapper remains a 100% effective match. Serialization helpers `0x004257E0`/`0x00425830`, remove helper `0x00425890`, Add helpers `0x00425C80`/`0x00425CE0`, manager cliff helper `0x00425DC0`, and deleting wrappers `0x004266E0`/`0x00426710` consume no portable symbols.

### CMaze path search

All 12 portable `CMaze` methods and the global `Direction` helper at `0x00423090..0x004238B0` now have genuine ownership in `src/AI/CMaze.cpp`, with the evidenced `0x103C` class layout and `tSolution` output record in `CMaze.h`. `Direction` is 100% exact; `BInitialise`, `BIteration`, and `BSolution` are behaviorally reconstructed at `67.48%`, `59.76%`, and `62.28%`. Macintosh-only `AdvanceBit`, `ChangeAddr`, and `ChangeNextAddr` remain unresolved pending standalone Windows identities.

### CMineManager

All 11 portable manager methods at `0x00424020..0x00424850` now have genuine ownership in `src/AI/CMineManager.cpp`: constructor, `Restart`, `Initialise`, destructor, `Triggered`, `Trigger`, `StepOn`, `Add`, `Process`, `GetViewData`, and `LoadLevel`; they match `100%` exact, `81.82%`, `64.52%`, `100%` exact, `85.71%`, `38.26%`, `54.05%`, `10.99%`, `28.00%`, `60.00%`, and `19.15%`. All 11 portable `CMine` methods now have genuine ownership in `src/AI/CMine.cpp`: constructor, `Initialise`, `Trigger`, `StepOn`, and `IsUsable` are exact; `Restart`, `DoActivate`, typed `Set(AICOORD)`, `SetTerrain`, `Process`, and `OnGround` match `85.71%`, `91.30%`, `65.12%`, `32.69%`, `97.10%`, and `40.00%`. Windows evidence preserves the `0x150` mine stride, six-byte tile-position cache, delayed chain threshold `< 0x801`, eight-pixel contact box, versioned slot IDs, and split CMine/manager ownership. Remove helper `0x004241A0` and deleting wrappers `0x00424C30`/`0x00424C60` consume no portable symbols.

### CGameObject movement/lift tail

Eleven core queue/facing/state methods now have genuine `CGameObject` ownership in `src/AI/CGameObjectMove.cpp`: `StopMoving`, `TurnToFaceDestination`, `FacingDestination`, `DeleteFirstEntryFromDestinationList`, `AddDestination`, `GetDestination`, `DestinationExists`, `EmptyDestinationList`, `GetBoundingBox`, `IsSelectable`, and `ResetInstructions` match `88.89%`, `97.62%`, `96.43%`, `83.15%`, `100%`, `100%`, `100%`, `100%`, `100%` effective, `54.55%`, and `100%`. `CGameObject::Init`, `GetId`, `SetId`, `ReSetId`, `NextId`, `NextLoadingId`, `RegisterId`, and `UpdateCollision` now have genuine ownership in `src/AI/CGameObjectId.cpp` and `src/AI/CGameObjectCollision.cpp`, matching `34.48%`, `100%`, `75.00%`, `100%`, `100%`, `100%`, `38.10%`, and `100%` respectively. Both neutral managed-entity getter/setter owners, both neutral allocator owners, the registration compatibility path, and the neutral collision-bounds view are deleted; the required getter ILT and every reconstructed caller route directly through genuine `CGameObject` methods. The genuine `CGameObject::StartSommersault` member in `src/AI/LVRANDVT.CPP` and both overloaded `OffLift` virtual forwarders in `src/AI/CGameObjectLift.cpp` are `100%` matches. The corresponding genuine point and rectangle `OnLift` overloads are reconstructed at `43.59%` and `43.21%`, preserving bounds, tile-height sampling, and fixed-point height updates. Their shared layout preserves the recovered action-phase/deadline fields, one-word member pointers, virtual slots, and `RET 4`/`RET 8` ABIs. Two additional `CODE_06_AI.bin` methods have high-confidence Windows identities in the mixed `LEVELVT.CPP` region: `Jump` at `0x00416130` and `Fall` at `0x00416220`; their movement bodies preserve trajectory, terrain, landing, and lift/contact behavior. `0x00416050` remains a Windows-only move-command queue helper: Macintosh `Blocked` is a distinct flag update and stays unresolved.

### CObjectManager

All 14 portable GMOB manager methods map at `0x0041AF60..0x0041BF00` in physical `LINKSCF.CPP`. Genuine `CObjectManager::CObjectManager(CAI*, int)`, `Restart`, `Initialise`, destructor, `AddObject(unsigned short, CGlobalGameObject*, unsigned char)`, `Process()`, `GetViewData(CViewData*)`, `FindObject(int)`, `Remove(CGlobalGameObject*)`, `ConvertVer0ToVer1()`, and `Receive(unsigned short, CGlobalGameObject*, CNetworkMessage*)` now replace their neutral GMOB-manager owners; the constructor, `Process`, `FindObject`, and `Receive` are `100%` matches, `Restart` and the destructor are `100%` effective, `Remove` is `96.30%`, `AddObject` is `88.89%`, `Initialise` is `75.86%`, `ConvertVer0ToVer1` is `70.37%`, and `GetViewData` is behaviorally reconstructed at `62.22%`. The typed vtable retains VC4's one-word member-pointer representation, and the constructor's placement-new caller in `CGame.cpp` uses the evidenced class directly. Windows evidence anchors the `0x40` manager layout, `CAI +0x158` ownership, pointer-table lifecycle, linked type-`0x11` creation, type-`0x14` switch creation/conversion, active-only lookup/removal, versioned loader, and network command `0x2A`. Windows-only bulk cleanup `0x0041B160`, recursive removal `0x0041B1B0`, integer-coordinate adapter `0x0041B310`, ID/slot helpers `0x0041B8A0..0x0041B990`, spatial queries `0x0041B9F0..0x0041BB10`, and deleting wrapper `0x0041C330` consume no portable symbols.
...
Genuine `CGameObject::Restart` and `Initialise` ownership now lives in `src/AI/CGameObjectLifecycle.cpp`; `Restart` is `80.49%` and `Initialise` is an exact match. The former neutral runtime-state view is reduced to the one Windows ABI bridge still required by an existing caller.

All twelve portable bodies map at `0x00416D20..0x00417150`. Exact 68K and Windows evidence agrees on lifecycle state, both `Action` overloads, request/cancel behavior, packet switch `0x23..0x29`, remove/cancel sends, and construction/destruction of nine shared message helpers. Genuine `CGlobalGameObject::Restart`, `UsableState`, `CancelRequest`, `SendRemove`, and `SendCancel` ownership now lives in `src/AI/CGlobalGameObject.cpp`; `Restart` is `91.67%` and the other four are exact matches. `UsableState` returns the pre-clear value; `CancelRequest` clears the evidenced inherited and derived request fields; and the send helpers bind the shared packet streams. The remaining bodies retain physical `LEVELRUN.CPP` ownership pending extraction. Windows-only `RequestAction(eAction,int)` adapter `0x00416E00` consumes no Macintosh symbol.
...
The three portable methods map consecutively at `0x0041CF70`, `0x0041D000`, and `0x0041D010`. Shared narrow `CTower` layout lives in `CTower.h`; genuine `Activate` lives in `LVPREDVT.CPP`, while genuine `ActivatePosition` lives in `LEVELVT.CPP`; both are `100%` matches. Exact 68K and Windows bodies agree on terrain-height refresh, unconditional activation success, and activation position `(x-0x30000,y-0x8000,z)`. Their ILTs occupy one Windows vtable at slots `+0x14`, `+0x18`, and `+0x2C`; neighboring CCrate and GMOB type-`0x14` bodies consume no CTower symbols.

## CBalloon reconstruction

The five portable methods map at `0x0041D600..0x0041D7B0`. Shared narrow `CBalloon` layout lives in `CBalloon.h`; genuine `Restart`, `Activate`, and `DoActivate` live in `LVSAVEPOS.CPP`, while genuine `ActivatePosition` lives in `LVPOSVT.CPP`; all four are `100%` matches. Exact 68K and Windows bodies agree on randomized timestamp restart, terrain refresh, network action `0x2A`, state `0x1A`, target permission/callback dispatch, score `50`, and unchanged activation-position output. Typed one-word member-pointer slots preserve the target permission, callback, and action dispatch ABI; the output method preserves the Windows hidden-result-pointer ABI.

## CCrate reconstruction

All six portable methods map at `0x0041CCA0..0x0041CF10`. Shared `CCrate` layout lives in `CCrate.h`; genuine `Restart` and `Activate` remain exact in `LVSAVEPOS.CPP`, while genuine `DoActivate` and `ActivatePosition` live in `LEVELVT.CPP` at `65.71%` and `96.55%`. Independent exact-body auditing confirms saved-position restart, idle-state timers `0x10/0x1E`, linked-content release through the GMOB manager, state `0x19/0x1A` processing, timer/action `0x14` behavior, contents-dependent scoring, and subtype-selected activation offsets. The Windows object uses linked object/id fields at `+0x148/+0x14C`; `ActivatePosition` preserves its hidden-result-pointer ABI.

## CCatapult reconstruction

All five portable methods map at `0x0041C700..0x0041CA60`. Independent auditing identifies the type-`4` vtable at `0x00494640` and confirms exact state, clock, randomized launch-vector, linked-target, owner-transfer, score-`20`, and activation-offset behavior. Shared `CCatapult` layout lives in `CCatapult.h`; genuine `Restart` and `Activate` are exact in `LVSAVEPOS.CPP`, while genuine `DoActivate` and `ActivatePosition` remain in `LEVELVT.CPP` at `25.00%` and `41.67%` due VC4 translation-unit register allocation. Shared helpers `0x0041C6C0` and `0x0041C670` remain target-only. `Process` remains deferred until its tile/grid and launch-vector boundaries have shared ownership.

## CAmmo reconstruction

All five portable methods map at `0x0041CA90..0x0041CC70` on the coherent Windows vtable at `0x00494268`: slots `5/6/11/65/67` hold Process, Activate, ActivatePosition, Restart, and DoActivate. Independent auditing corrected the preliminary crossed-family hypothesis: `0x0041C700/0x0041C720` belong to CCatapult, while CAmmo uses `0x0041CA90/0x0041CAB0`. Shared `CAmmo` layout lives in `CAmmo.h`; genuine `Restart` and `Activate` remain in `LVSAVEPOS.CPP` at `100%` and `90.57%`, genuine `DoActivate` lives in `LEVELVT.CPP` at `92.68%`, and genuine `ActivatePosition` remains exact in `LVPOSVT.CPP`. Exact behavior agrees on the class-specific duration field, idle acceptance, deadline `8`, state request `0x1A`, action `0x11`, target action `0x19`, score `50`, and unchanged hidden-result position output.
...
All twenty-five portable network-message methods map across `0x0045EE80..0x0045F2B0` with a genuine `CNetworkMessage` class declaration in `src/Visos/Generic/CNetworkMessage.h` and mixed `EFFSTRM.CPP` and `VSNETSTR.CPP` implementation ownership. Nineteen of the twenty-five are exact (100%) matches; the remaining six (string/range `Add`, `OpenDataStream`, `CopyDataStream`, range `GetCopy`, and `Send`) differ primarily in the expanded Windows stream layout, endian-aware cursor state, and transport dispatch indirection. Exact big-endian append/read/convert behavior, pointer aliases, copy semantics, expanded layout, vtables and send-event path support the family. Deleting wrappers, a duplicate destructor thunk, and the following request-control streams remain target-only or separate.
