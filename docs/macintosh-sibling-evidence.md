...
All five portable `CFormationManager` methods align with Windows `0x0041A140..0x0041A320` through the 0x104-byte three-block vector table, fixed16.12 initialization, signed sine/cosine rotation of eight vectors, transformed output at `+0xC0`, and wrapped accessor. Genuine `Restart` and destructor ownership now lives in `src/AI/CFormationManager.cpp` as `100%` matches, with the original constructor ILT redirected to `Restart`. Windows iterator helpers `0x0041A2E0` and `0x0041A300` remain separate.
