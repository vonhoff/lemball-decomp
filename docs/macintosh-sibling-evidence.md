...
Ten core queue/facing/state methods now have genuine `CGameObject` ownership in `src/AI/CGameObjectMove.cpp`: `StopMoving`, `TurnToFaceDestination`, `FacingDestination`, `DeleteFirstEntryFromDestinationList`, `AddDestination`, `GetDestination`, `DestinationExists`, `EmptyDestinationList`, `GetBoundingBox`, and `IsSelectable` match `88.89%`, `97.62%`, `96.43%`, `83.15%`, `100%`, `100%`, `100%`, `100%`, `100%` effective, and `54.55%`. `CGameObject::GetId`, `SetId`, `ReSetId`, `NextId`, `NextLoadingId`, `RegisterId`, and `UpdateCollision` now have genuine ownership in `src/AI/CGameObjectId.cpp` and `src/AI/CGameObjectCollision.cpp`, matching `100%`, `75.00%`, `100%`, `100%`, `100%`, `38.10%`, and `100%` respectively. Both neutral managed-entity getter/setter owners, both neutral allocator owners, the registration compatibility path, and the neutral collision-bounds view are deleted; the required getter ILT and every reconstructed caller route directly through genuine `CGameObject` methods. Five additional `CODE_06_AI.bin` methods have high-confidence Windows identities in the mixed `LEVELVT.CPP` region: `Jump` at `0x00416130`, `Fall` at `0x00416220`, the one- and two-reference `OffLift` forwarders at `0x00416410` and `0x004164F0`, and member `StartSommersault` at `0x00416510`. The movement bodies preserve trajectory, terrain, landing, and lift/contact behavior; the forwarders preserve vtable slots and `RET 4`/`RET 8`; `StartSommersault` preserves the exact RNG/deadline/phase algorithm. `0x00416050` remains a Windows-only move-command queue helper: Macintosh `Blocked` is a distinct flag update and stays unresolved.
