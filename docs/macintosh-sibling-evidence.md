...
The 12 portable `CMaze` methods and global `Direction` map at `0x00423090..0x004238B0`. Windows evidence preserves the `0x103C` layout, row-allocated 16-bit passability/distance grid, two fixed-stride frontier banks, edge-mask-constrained nine-neighbor relaxation, reference arguments, bounded breadth expansion, direction coding, and turn-only solution output. The first eleven portable members through `CMaze::BIteration(unsigned char&, unsigned char&)` and the global `Direction` helper now have genuine ownership in `src/AI/CMaze.cpp`; `Direction` is 100% exact, while `BInitialise` and `BIteration` are behaviorally reconstructed at `67.48%` and `59.76%`. Only `BSolution` remains outside the reconstructed maze translation unit. Macintosh-only `AdvanceBit`, `ChangeAddr`, and `ChangeNextAddr` remain unresolved pending standalone Windows identities.
