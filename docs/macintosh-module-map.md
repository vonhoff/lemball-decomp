...
| `AI` | `CDoor`, `CDoorManager`, `CIce`, `CIceManager`; remaining game objects, players, enemies | `src/AI/CDoorAct.cpp`, `CDoorSlot.cpp`, `CDoorCtor.cpp`, `CIceAct.cpp`, `CInvsMgr.cpp`, `CInvsSupp.cpp`, `CLiftAct.cpp`, `CLiftRun.cpp`; mixed Windows owners retained elsewhere | High for Door and Ice correlations; `CInvsMgr.cpp` and `CInvsSupp.cpp` are coherent invisible-switch manager/object physical fragments, with provisional labels retained pending call-shape-safe correlation; Lift activation and processing fragments are likewise physically coherent; `LINKSCF.CPP` remains a 296-function mixed Windows owner |
